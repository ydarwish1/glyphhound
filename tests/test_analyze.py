...
Phase 2 is the STRING/structural baseline: it flags the presence of section 4 sink patterns
...
# --- per-rule unit coverage (each maps to one section 4 catalog entry) -----------------
...
def test_flags_generic_attr_filter_pivot():
    # |attr with a non-dunder argument is still the dot-access dodge (section 4 pivot).
...
# --- section 7: scan ALL templates, tag each finding by its template name ---------------
...
def test_named_template_attack_is_caught_end_to_end(tmp_path):
    # The section 7 payoff: a model whose DEFAULT template is benign but a NAMED template
