...
GlyphHound is developed in small, verified stages ("phases"): each change is finished, proven with
real output, and only then is the next one started. Most phases have a re-runnable
`scripts/verify_phase*.py` that demonstrates them.
