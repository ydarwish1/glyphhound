# Head-to-head benchmark: GlyphHound vs ModelAudit
...
The obfuscated set covers nine techniques: three string-concat chains plus the `str.format`,
slice, `|join`, and `|replace` constant-builder families. GlyphHound catches all nine;
ModelAudit catches three, so the GlyphHound-only edge is six payloads (the concat-gated chains
plus the slice / `|join` / `|replace` builders).

`verify_phase8.py` re-measures the table live against the current analyzer and every verdict
above is unchanged. The analyzer is not tuned to the benchmark: the payloads exercise only a
slice of the sink catalog, so the measured rates hold as the catalog grows. The broader catalog
is instead audited for false positives on a wider benign sample (`study/wider_fp_audit.json`).
...
**`|join`**, and **`|replace`** constant-builder families. Two independent
