"""Stage 3 data model and sink catalog (ARCHITECTURE.md section 4 and section 5).

Phase 2 is the **string/structural baseline**: it flags the *presence* of section 4 sink
...
# finding is explainable and standards-mapped (ARCHITECTURE.md section 4).
RULE_CATALOG: dict[str, tuple[str, str, str, str]] = {
    ...
    "GH-S003": ("attr-filter", HIGH,
                "use of the |attr() filter to dodge dot-access detection (section 4 pivot)",
...
class Finding:
    """One detected sink (ARCHITECTURE.md section 5), tagged with its source template (section 7).
