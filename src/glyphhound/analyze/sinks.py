...
Walk the Jinja2 AST produced by Stage 2 and flag the section 4 sink catalog. This is the
...
this stage still *matches* the section 4 sink patterns, and :mod:`.taint` decides which are
...
def analyze_raw(raw: RawTemplate) -> list[Finding]:
    ...
    security payoff of the multi-template acquirer (ARCHITECTURE.md section 7): a sink hidden
...
def analyze_ast(ast: nodes.Node, *, template_name: str | None = None) -> list[Finding]:
    """Walk a parsed AST and return findings for the section 4 sink catalog.
...
def _inspect_attr_filter(node: nodes.Filter, template_name: str | None,
    ...
    """The ``|attr('...')`` filter (ARCHITECTURE.md section 4 pivot).
