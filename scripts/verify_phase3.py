...
Checks ("flags a reachable-sink fixture; does NOT flag a benign template that merely
names a variable like `class`"):
  ...
  (c) The real benign corpus (>=10 templates) stays at 0 reachable findings.
...
# The Phase-3 false-positive probe: a variable merely NAMED `class` is
# not a `__class__` access, so it builds toward nothing. Kept inline (mirrors the unit
# test) rather than as a benign corpus file, so the benign corpus stays the real set.
...
def verify_benign_corpus_zero_reachable() -> bool:
    print("\n" + "=" * 78)
    print("Phase 3 (c) -- real benign corpus must stay 0 reachable findings")
    print("=" * 78)
